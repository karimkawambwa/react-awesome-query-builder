...
import { SpelConcatPart } from "core";
...
const MltSelector: FC<Iprops> = ({ k, value, setValue }) => {
  function initMltSelectValueHandler(
    list: OptionItem[],
    val: SpelConcatPart[]
  ) {
    if (val) {
      return val.map((item: SpelConcatPart) => {
        let res =
          item.type != "const" &&
          list &&
          list.find((obj) => obj.id === item.value);
        if (!res) {
